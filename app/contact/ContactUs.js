    ...
    <>
      ...
      >
        <div className="container">
          ...
          {/* Image + Form / Global Presence Section */}
                ...
                {locations.length > 0 ? (
                  <ul className={Style.presence_list}>
                    {locations.map((loc, index) => (
                      <li key={index} className={Style.presence_item}>
                        <h5 className={Style.presence_title}>
                          {loc.flag?.url && (
                            <img
                              className={Style.presence_flag}
                              src={`${process.env.NEXT_PUBLIC_STRAPI_ASSETS_BASE_URL}${loc.flag.url}`}
                              alt={`${loc.location || "Office"} flag`}
                            />
                          )}
                          {loc.location}
                        </h5>
                        {loc.address && (
                          <p className={Style.presence_address}>
                            <RiMapPinLine />
                            <span>{loc.address}</span>
                          </p>
                        )}
                        <div className={Style.presence_contacts}>
                          {loc.email && (
                            <a href={`mailto:${loc.email}`} className={Style.presence_contact}>
                              <RiMailLine />
                              <span>{loc.email}</span>
                            </a>
                          )}
                          {loc.phoneNumber && (
                            <a href={`tel:${loc.phoneNumber}`} className={Style.presence_contact}>
                              <RiPhoneLine />
                  ...
                  <p className={Style.presence_fallback}>
