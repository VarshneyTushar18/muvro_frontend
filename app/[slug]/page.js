...
async function fetchSoftwareSlugs() {
    const res = await fetch(
...
async function getSoftware(slug) {
    const res = await fetch(
        `${process.env.STRAPI_BACKEND_BASE_URL}/software-pages?filters[slug][$eq]=${slug}&populate=*`,
        { next: { revalidate: 60 } }
    );
...
function SoftwareSinglePage({ software }) {
    const crumbs = [
        { label: "Home", link: "/" },
                        ...
                        >
                            <div className="position-sticky" style={{ top: "10px" }}>
                                <SwiperSlider
                                    ...
                                    galleryName={`${software.slug}-gallery`}
